"""
v0.6 LONG Retrain on 49-feature CSV (added bars_since_swing_high/low + pullback_depth_pct).

Multi-threshold walk-forward CV. Picks the threshold that maximizes worst-fold
expectancy among those passing Gate A. v0.5 LONG passed Gate A only at thr=0.60.
v0.6 hypothesis: structure features fix Fold 4 ("stale setup" failure mode).

Output:
  - eurusd_long_v06.joblib (only if Gate A passes at any threshold)
  - v06_retrain_results.md
"""
...
MODEL_NAME = 'eurusd_long_v06.joblib'
RESULTS_FILE = 'v06_retrain_results.md'
...
GATE_A = {'min_roc_auc': 0.55, 'min_positive_folds': 4, 'min_expectancy_r': 0.09}

print("=" * 70)
print("v0.6 LONG Retrain (49 features)")
print("=" * 70)
...
if best_threshold is not None:
    ...
    save_model(final_model, str(model_path), metadata={
        'symbol': 'EURUSD', 'direction': 'long', 'version': 'v06',
...
# Markdown report
print("\n" + "=" * 70)
print(f"v0.6 LONG SUMMARY — Gate A: {gate_a_status}")
print("=" * 70)

with open(RESULTS_FILE, 'w') as f:
    f.write("# v0.6 LONG Retrain Results\n\n")
    f.write(f"**Date:** {datetime.now().isoformat()}\n")
    f.write(f"**Status:** {gate_a_status}\n")
    f.write(f"**Features:** {len(features)} (added bars_since_swing_high/low + pullback_depth_pct)\n\n")
