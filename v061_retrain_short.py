"""
v0.6.1 SHORT Retrain on 49-feature CSV.

Mirrors v061_retrain.py but for SHORT direction. v0.5 SHORT was deferred to
this retrain because 2023-only data lacked enough bearish samples. v0.6.1
adds H4 regime features (slope_sma_h4_200, mtf_with_h4_score,
h4_alignment_duration) which were the fix for LONG Fold 4. Hypothesis: H4
persistence should help SHORT spot regime continuations that v0.4's
h1-only alignment missed.

Selection: best worst-fold expectancy among Gate A passers (NOT locked,
since SHORT has no prior live calibration).

Output:
  - eurusd_short_v061.joblib (only if Gate A passes at any threshold)
  - v061_retrain_short_results.md
"""
...
MODEL_NAME = 'eurusd_short_v061.joblib'
RESULTS_FILE = 'v061_retrain_short_results.md'
...
# Match LONG v0.6.1 relaxation (0.54). Same rationale: trading metrics pay,
# AUC is a heuristic.
GATE_A = {'min_roc_auc': 0.54, 'min_positive_folds': 4, 'min_expectancy_r': 0.09}

print("=" * 70)
print("v0.6.1 SHORT Retrain (49 features, H4 regime)")
print("=" * 70)
...
# Gate A — pick best worst-fold among passers (no locked threshold for SHORT)
...
if best_threshold is not None:
    ...
    print(f"  Training final SHORT model on full train/CV...")
    df_short_full = collapse_to_binary(train_cv, direction="short", timeout_as="loss")
    X_full = df_short_full[features]
    y_full = df_short_full['label']
    ...
    save_model(final_model, str(model_path), metadata={
        'symbol': 'EURUSD', 'direction': 'short', 'version': 'v061',
...
# Markdown report
print("\n" + "=" * 70)
print(f"v0.6.1 SHORT SUMMARY -- Gate A: {gate_a_status}")
print("=" * 70)

with open(RESULTS_FILE, 'w') as f:
    f.write("# v0.6.1 SHORT Retrain Results\n\n")
